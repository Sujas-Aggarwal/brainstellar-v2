...
export default function About() {
  return (
    <div className="pt-24 pb-32 px-6 max-w-4xl mx-auto min-h-screen bg-(--bg) text-(--fg)">
      <Navbar />
      
      <header className="mb-24 space-y-10">
        <Link to="/" className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-(--muted-fg) hover:text-(--fg) transition-colors">
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs font-bold text-(--muted-fg) uppercase tracking-[0.4em]">100+ High-Quality Logic Challenges</p>
        </div>
      </header>
      ...
      <section className="space-y-32">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-20">
          <div className="space-y-8">
            <h2 className="text-2xl font-bold tracking-tight uppercase border-b-2 border-(--fg) pb-4 inline-block">The Goal</h2>
            <p className="text-lg font-medium leading-relaxed text-(--muted-fg)">
              Brainfuck is designed to be the ultimate resource for <span className="text-(--fg)">interview preparation</span>. We provide free access to complex puzzles that are frequently asked in Quant, HFT, and SDE interviews at top firms.
            </p>
          </div>
          <div className="space-y-8">
            <h2 className="text-2xl font-bold tracking-tight uppercase border-b-2 border-(--fg) pb-4 inline-block">The Legacy</h2>
            <p className="text-lg font-medium leading-relaxed text-(--muted-fg)">
              We credit the legendary <a href="https://brainstellar.com" target="_blank" rel="noopener noreferrer" className="text-(--fg) underline underline-offset-4 font-bold">Brainstellar.com</a> as our primary source of inspiration. We have updated and curated this collection to help modern developers and quantitative analysts succeed in their job search.
            </p>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-(--border) border border-(--border)">
          <div className="p-12 bg-(--bg) space-y-6">
            <Zap className="w-8 h-8 text-(--c-medium)" />
          ...
          <div className="p-12 bg-(--bg) space-y-6">
            <Shield className="w-8 h-8 text-(--c-easy)" />
            <h3 className="text-xl font-bold uppercase tracking-tight">Privacy & Sync</h3>
            <p className="text-sm text-(--muted-fg) leading-relaxed">
              We collect minimal, anonymous analytics to improve the site. Log in to securely sync your progress across devices, or continue as a guest with local storage.
            </p>
          </div>
          <div className="p-12 bg-(--bg) space-y-6">
            <Rocket className="w-8 h-8 text-(--c-hard)" />
            <h3 className="text-xl font-bold uppercase tracking-tight">High Fidelity</h3>
            <p className="text-sm text-(--muted-fg) leading-relaxed">
          ...
          <div className="p-12 bg-(--bg) space-y-6">
            <Globe className="w-8 h-8 text-(--c-overall)" />
        ...
        <footer className="pt-20 border-t border-(--border) space-y-12">
          <div className="space-y-6 text-center max-w-2xl mx-auto">
            <h4 className="text-sm font-bold uppercase tracking-[0.5em] text-(--fg)">Start Your Prep</h4>
            <p className="text-sm text-(--muted-fg) leading-relaxed">
